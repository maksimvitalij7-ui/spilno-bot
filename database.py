...
class Database:
    ...
    def save_salary_info(self, telegram_id: int, salary_date: str, salary_amount: str, bonus_info: str):
        self.conn.execute("""
            INSERT INTO salary_info (telegram_id, salary_date, salary_amount, bonus_info)
            VALUES (?, ?, ?, ?)
            ON CONFLICT(telegram_id) DO UPDATE SET
        ...
        """, (telegram_id, salary_date, salary_amount, bonus_info))
        self.conn.commit()

    def get_salary_info(self, telegram_id: int):
        row = self.conn.execute(
            "SELECT * FROM salary_info WHERE telegram_id = ?", (telegram_id,)
        ).fetchone()
        return dict(row) if row else None

    def get_all_salary_info(self):
        return [dict(r) for r in self.conn.execute("""
            SELECT e.name, e.username, s.salary_date, s.salary_amount, s.bonus_info
            FROM employees e
            LEFT JOIN salary_info s ON e.telegram_id = s.telegram_id
            WHERE e.is_admin = 0
            ORDER BY e.name
        """).fetchall()]
