...
async def handle_report_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("salary_step") == "bonus":
        ...
        await update.message.reply_text(
            ...
            f"💵 Ставка: {salary_amount}\n"
            f"🎁 Бонусы: {bonus_info}\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "Каждый день я буду присылать тебе:\n"
            "🌅 Утром (09:00) — отметка о приходе\n"
            "🌆 Вечером (18:00) — отчёт за день\n\n"
...
async def check_salary_day(context: ContextTypes.DEFAULT_TYPE):
    """В день зарплаты вечером спрашивает сотрудника получил ли он зарплату"""
...
def main():
    ...
    job_queue.run_daily(send_morning_checkin,     time=time(9, 0, tzinfo=tz))   # 09:00
    job_queue.run_daily(send_reminder,            time=time(10, 30, tzinfo=tz)) # 10:30 напоминание
